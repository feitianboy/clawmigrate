...
exports.queryZpayOrder = queryZpayOrder;
exports.zpayRefund = zpayRefund;
exports.buildZpayPaymentUrl = buildZpayPaymentUrl;
exports.getZpayConfig = getZpayConfig;
...
function generateSign(params, key) {
    ...
    return crypto_1.default.createHash('md5').update(signStr, 'utf8').digest('hex');
}
function verifySign(params, key, receivedSign) {
    const sortedKeys = Object.keys(params)
        .filter(k => k !== 'sign' && k !== 'sign_type' && params[k] !== '')
        .sort();
    const queryString = sortedKeys.map(k => k + '=' + params[k]).join('&');
    const signStr = queryString + key;
    const calculatedSign = crypto_1.default.createHash('md5').update(signStr, 'utf8').digest('hex');
...
async function zpayRefund(orderId, amount) {
    try {
...
function buildZpayPaymentUrl(orderId, planName, amount, payType) {
    const zpayParams = {
        pid: ZPAY_PID,
        type: payType,
        out_trade_no: orderId,
