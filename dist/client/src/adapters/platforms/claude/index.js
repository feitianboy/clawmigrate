...
exports.claudeAdapter = {
    ...
    supportedImportCategories: [
        ...
        types_1.MigrationCategory.SETTINGS,
        types_1.MigrationCategory.PROMPTS,
        types_1.MigrationCategory.SKILLS, // 其他平台的技能 → Claude Projects
        types_1.MigrationCategory.AUTOMATIONS, // 其他平台的自动化 → Claude 自定义指令
    ],
...
- 系统提示词如果太长，可以概括性描述，但尽量完整`;
    ...
    parseExportResult(raw) {
        ...
        if (rawSettings) {
            const customSettings = {
                ...
                ...(rawSettings.custom_settings || {}),
            };
            const sensitivity = (0, utils_1.detectSensitivity)(customSettings);
            settings = {
            ...
            if (sensitivity !== types_1.SensitivityLevel.SAFE) {
                sensitiveItems.push({
                    ...
                    maskedValue: (0, utils_1.maskSensitiveData)(JSON.stringify(customSettings)),
                });
            }
        }
    ...
    generateImportPrompt(schema, options) {
        ...
        if (configs.settings && options.categories.includes(types_1.MigrationCategory.SETTINGS)) {
            ...
            if (s.model)
                parts.push(`- 默认模型：${s.model}（对应 Claude 的 Model Settings）`);
            if (s.temperature !== undefined)
                parts.push(`- 温度：${s.temperature}（对应 Claude 的 Temperature）`);
            if (s.language)
                parts.push(`- 语言：${s.language}（对应 Claude 的 Language Preference）`);
            if (s.personaDescription) {
                parts.push('\\n## 🎭 人设/角色设定');
                parts.push('请按照以下人设描述设置你的角色（对应 Claude 的 Persona）：');
                parts.push(s.personaDescription);
            }
            if (s.customSettings.effortLevel)
                parts.push(`- 思考量：${s.customSettings.effortLevel}（对应 Claude 的 Effort Level）`);
