...
async function handleGetMigrations(req, res) {
    try {
        ...
        const { data: migrations } = await supabase_1.supabase.from('migrations').select('*').eq('user_id', result.user.id).order('created_at', { ascending: false });
...
async function handleCreateMigration(req, res) {
    try {
        const result = await (0, auth_1.requireAuth)(req);
        if (result.error)
            return res.status(result.error.status).json({ ok: false, error: result.error.message });
        ...
        if (!sourcePlatform || !targetPlatform)
            return res.status(400).json({ ok: false, error: 'Source and target platforms are required' });
        ...
        const { data: migration, error } = await supabase_1.supabase.from('migrations').insert({
        ...
        if (error || !migration)
            return res.status(500).json({ ok: false, error: 'Failed to create migration' });
        await (0, utils_1.logActivity)(result.user.id, 'migration', { migrationId: migration.id, sourcePlatform, targetPlatform, itemsCount: migration.items_count, status: migration.status }, req.headers['x-forwarded-for'] || req.ip);
...
async function handleUpdateMigration(req, res, idStr) {
    try {
        const result = await (0, auth_1.requireAuth)(req);
        if (result.error)
            return res.status(result.error.status).json({ ok: false, error: result.error.message });
        ...
        const { data: migration } = await supabase_1.supabase.from('migrations').select('*').eq('id', migrationId).single();
        if (!migration)
            return res.status(404).json({ ok: false, error: 'Migration not found' });
        if (migration.user_id !== result.user.id)
        ...
        const { data: updated, error } = await supabase_1.supabase.from('migrations').update({ status }).eq('id', migrationId).select().single();
        if (error || !updated)
            return res.status(500).json({ ok: false, error: 'Failed to update migration' });
        await (0, utils_1.logActivity)(result.user.id, 'migration_update', { migrationId, status }, req.headers['x-forwarded-for'] || req.ip);
...
async function handleGetMigration(req, res, idStr) {
    try {
        const result = await (0, auth_1.requireAuth)(req);
        if (result.error)
            return res.status(result.error.status).json({ ok: false, error: result.error.message });
