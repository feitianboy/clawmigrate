...
function countItems(data) {
    let count = 0;
    if (data.skills)
        count += Array.isArray(data.skills) ? data.skills.length : 1;
    if (data.memories)
        count += Array.isArray(data.memories) ? data.memories.length : 1;
    if (data.projects)
        count += Array.isArray(data.projects) ? data.projects.length : 1;
    if (data.mcp_connections)
        count += Array.isArray(data.mcp_connections) ? data.mcp_connections.length : 1;
    if (data.settings)
        count += 1;
    return count;
}
function generateImportPrompt(data, targetInfo, totalItems) {
    const agentName = data.agent_name || data.name || '迁移的 Agent';
    let prompt = `请帮我创建一个名为 "${agentName}" 的配置，包含以下内容：\n\n`;
    if (data.settings) {
        prompt += `【系统设置】\n`;
        if (data.settings.model)
            prompt += `- 模型：${data.settings.model}\n`;
        if (data.settings.temperature !== undefined)
            prompt += `- 温度：${data.settings.temperature}\n`;
        if (data.settings.language)
            prompt += `- 语言：${data.settings.language}\n`;
    ...
    if (data.mcp_connections && Array.isArray(data.mcp_connections)) {
        ...
        data.mcp_connections.forEach((conn) => {
