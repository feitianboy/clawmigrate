...
const sampleData_1 = require("../../lib/sampleData");
...
async function handleGetAgents(req, res) {
    ...
    const sampleData = sampleData_1.sampleExports[platformId];
    const agents = [];
    if (sampleData) {
        if (sampleData.projects && Array.isArray(sampleData.projects)) {
            sampleData.projects.forEach((p, i) => {
                agents.push({
                    id: `project_${i}`,
...
async function handleFetchConfig(req, res) {
    if (req.method !== 'POST') {
        return res.status(405).json({ ok: false, error: 'Method not allowed' });
    }
...
async function handleGetSkillConfig(req, res) {
    if (req.method !== 'GET') {
        return res.status(405).json({ ok: false, error: 'Method not allowed' });
    }
