...
function verifyOAuthState(state) {
    try {
        ...
        if (Date.now() - timestamp > 10 * 60 * 1000)
            return false;
        const expectedHmac = crypto_1.default.createHmac('sha256', secret).update(`${timestamp}:${random}`).digest('hex');
        return crypto_1.default.timingSafeEqual(Buffer.from(receivedHmac), Buffer.from(expectedHmac));
    }
    catch {
        return false;
    }
...
async function handler(req, res) {
    ...
    if (subPath === 'admin-login' && req.method === 'POST')
        return handleAdminLogin(req, res);
    if (subPath === 'callback' && req.method === 'GET')
        return handleGithubCallback(req, res);
    if (subPath === 'check' && req.method === 'GET')
...
async function handleGithubCallback(req, res) {
    ...
    const { data: existingUser } = await supabase_1.supabase.from('users').select('*').eq('username', githubUser.login).single();
    let user = existingUser;
    if (!existingUser) {
        const { data: newUser, error } = await supabase_1.supabase.from('users').insert({
    ...
    await (0, utils_1.logActivity)(user.id, 'github_login', { username: user.username }, (0, auth_1.getClientIp)(req));
...
async function handleLogin(req, res) {
    ...
    // 速率限制
    const rateLimitKey = `login:${(0, auth_1.getClientIp)(req)}`;
    ...
    const { data: user } = isEmail
        ? await supabase_1.supabase.from('users').select('id, username, email, role, password_hash').eq('email', username).single()
        : await supabase_1.supabase.from('users').select('id, username, email, role, password_hash').eq('username', username).single();
    if (!user || !user.password_hash || !bcryptjs_1.default.compareSync(password, user.password_hash)) {
        (0, auth_1.recordFailedAttempt)(rateLimitKey);
        return res.status(401).json({ ok: false, error: '用户名或密码错误' });
    }
    (0, auth_1.clearRateLimit)(rateLimitKey);
    await (0, utils_1.logActivity)(user.id, 'login', { username: user.username }, (0, auth_1.getClientIp)(req));
    const secret = process.env.JWT_SECRET;
    if (!secret)
        return res.status(500).json({ ok: false, error: 'Server authentication not configured' });
    const token = jsonwebtoken_1.default.sign({ userId: user.id, username: user.username, role: user.role }, secret, { expiresIn: '7d' });
...
async function handleRegister(req, res) {
    ...
    const { data: existingUser } = await supabase_1.supabase.from('users').select('id').eq('username', username).single();
    if (existingUser)
        return res.status(400).json({ ok: false, error: 'Username already exists' });
    const { data: existingEmail } = await supabase_1.supabase.from('users').select('id').eq('email', email).single();
    if (existingEmail)
        return res.status(400).json({ ok: false, error: 'Email already exists' });
    const passwordHash = bcryptjs_1.default.hashSync(password, SALT_ROUNDS);
    const { data: newUser, error } = await supabase_1.supabase.from('users').insert({
        username, email, password_hash: passwordHash, role: 'user'
    }).select('id, username, email, role').single();
    if (error || !newUser)
        return res.status(500).json({ ok: false, error: 'Failed to create user' });
    await (0, utils_1.logActivity)(newUser.id, 'register', { username: newUser.username }, (0, auth_1.getClientIp)(req));
    const secret = process.env.JWT_SECRET;
    if (!secret)
        return res.status(500).json({ ok: false, error: 'Server authentication not configured' });
    const token = jsonwebtoken_1.default.sign({ userId: newUser.id, username: newUser.username, role: newUser.role }, secret, { expiresIn: '7d' });
    res.setHeader('Set-Cookie', `token=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=604800`);
...
async function handleAdminLogin(req, res) {
    const { username, password } = req.body;
    if (!username || !password)
        return res.status(400).json({ ok: false, error: '账号和密码为必填' });
    // 速率限制
    const rateLimitKey = `admin:${(0, auth_1.getClientIp)(req)}`;
    const rateLimit = (0, auth_1.checkRateLimit)(rateLimitKey);
    if (!rateLimit.allowed) {
        return res.status(429).json({ ok: false, error: `尝试过于频繁，请 ${rateLimit.retryAfter} 秒后再试` });
    }
    // 从 admins 表查询管理员
    const { data: admin } = await supabase_1.supabase.from('admins').select('id, username, password_hash').eq('username', username).single();
    if (!admin || !admin.password_hash || !bcryptjs_1.default.compareSync(password, admin.password_hash)) {
        (0, auth_1.recordFailedAttempt)(rateLimitKey);
