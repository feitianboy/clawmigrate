    ...
    try {
      ...
      return {
        ...
        errors: [{
          ...
          suggestion: '请确保粘贴的是完整的 JSON 结果，不要包含额外文字。常见问题：被 markdown 代码块包裹、多余文字前缀。',
        }],
        warnings: [],
      };
    }
    ...
    if (rawSkills && Array.isArray(rawSkills)) {
      rawSkills.forEach((s, i) => {
        const skill: SkillConfig = {
          ...
          originalFieldNames: { name: '插件名称', description: '插件描述', type: '插件类型', config: '插件配置', enabled: '是否启用' },
        };
    ...
    if (rawAutomations && Array.isArray(rawAutomations)) {
      rawAutomations.forEach((a, i) => {
        automations.push({
          ...
          originalFieldNames: { name: '工作流名称', description: '工作流描述', type: '类型', trigger: '触发条件', actions: '执行动作', schedule: '定时规则', enabled: '是否启用' },
        });
      });
    }
  ...
  getFieldMapping(): FieldMappingTable {
    return {
      skills: {
        ...
        description: { unifiedField: 'description', displayName: '插件描述', type: 'string', required: false },
        type: { unifiedField: 'type', displayName: '插件类型', type: 'string', required: true, description: 'plugin/skill/tool/action' },
        config: { unifiedField: 'config', displayName: '插件配置', type: 'object', required: true, sensitivityLevel: SensitivityLevel.REVIEW_SUGGESTED },
      ...
      automations: {
        ...
        description: { unifiedField: 'description', displayName: '工作流描述', type: 'string', required: false },
        type: { unifiedField: 'type', displayName: '类型', type: 'string', required: true, description: 'schedule/trigger/workflow' },
      ...
      mcp_connections: {
        ...
        server_url: { unifiedField: 'serverUrl', displayName: '服务器地址', type: 'string', required: false, sensitivityLevel: SensitivityLevel.REVIEW_SUGGESTED },
        transport_type: { unifiedField: 'transportType', displayName: '传输方式', type: 'string', required: false, description: 'stdio/sse/streamable-http' },
        tools: { unifiedField: 'tools', displayName: '工具列表', type: 'array', required: false },
        config: { unifiedField: 'config', displayName: '配置详情', type: 'object', required: true, sensitivityLevel: SensitivityLevel.MUST_REMOVE },
        enabled: { unifiedField: 'enabled', displayName: '是否启用', type: 'boolean', required: false },
      },
      memories: {
        content: { unifiedField: 'content', displayName: '记忆内容', type: 'string', required: true },
        type: { unifiedField: 'type', displayName: '记忆类型', type: 'string', required: true, description: 'fact/preference/instruction/context' },
      ...
      knowledge_bases: {
        ...
        source_type: { unifiedField: 'sourceType', displayName: '来源类型', type: 'string', required: false, description: 'upload/url/api' },
      },
    };
  },
};
